...
type cellChannels struct {
	coordinates chan cell
...
func main() {
	...
	runeChan := make(chan rune) //added
	go getKeyboardCommand(runeChan) //send channel to function
	gameOfLife(params, runeChan)
	StopControlServer()
}
